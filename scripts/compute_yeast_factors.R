...
#
# sinteractive --mem=16G -c 8 --time=24:00:00 -p mstephens \
...
t0 <- proc.time()
tm0 <- fit_poisson_nmf(counts,k = 15,numiter = 20,method = "em",
                      control = list(numiter = 4,nc = 8,extrapolate = FALSE),
                      init.method = "random",verbose = "detailed")
tm <- fit_poisson_nmf(counts,fit0 = tm0,numiter = 20,method = "scd",
...
fl_nmf <- flashier_nmf(shifted_log_counts,k = 15,greedy_init = TRUE,
                       var_type = 2,S = s1,verbose = 2,maxiter = 20)
t1 <- proc.time()
