...
clusters <- factor(clusters,
                   c("beta","alpha","delta+PP","acinar","ductal",
				     "macrophages","endothelial","mesenchymal"))
```

TO DO (to continue improving the Structure plots):

+ Show the plots by cell type first.
...
set.seed(1)
factor_colors <-
  c("lightgray","limegreen","darkviolet","dodgerblue","darkorange","dimgray",
    "gold","darkblue","forestgreen","tomato","orchid","red",
    "olivedrab")
...
colnames(L) <- paste0("k",1:13)
set.seed(1)
rows <- c(sample(which(clusters == "beta"),600),
          sample(which(clusters != "beta"),1000))
rows <- sort(rows)
...
Additional plots showing that factor 5 clearly captures responses to
...
Structured plots in which the cells are grouped by cell type:
...
```{r structure-plots-flashier-alpha, fig.height=1.5, fig.width=5, results="hide", message=FALSE}
set.seed(1)
factors <- c(1,6,5,12,13,4,3)
...
NEXT: Add Structure plot(s) for beta cells, delta+PP cells, and then
for the other cells.
...
+ Add back Structure plot for k=15 to show that adding more
  factors identifies the individual conditions, but it
  arguably less biologically interesting.

+ Add a UMAP plot and visualize the clusters and conditions
  on this UMAP plot.
...
Structure plot for k=15 result:

```{r structure-plots-flashier-by-treatment-k-15, fig.height=4.5, fig.width=7.5, results="hide", message=FALSE}
set.seed(1)
load("../output/pancreas_lsa_factors_k=15.RData")
factor_colors <-
  c("#000000","#FFB300","#803E75","#FF6800","#A6BDD7","#C10020",
    "#CEA262","#817066","#007D34","#F6768E","#00538A","#FF7A5C",
    "#53377A","#FF8E00","#B32851")
cell_type_factors <- c(2:4,7:11,13)
treatment_factors <- c(5,6,12)
...
TO DO: Fix the colors in these Structure plots to match the k=13
Structure plots.
