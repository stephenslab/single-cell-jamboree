...
```{r gene-matrix-scatterplots, fig.height=2, fig.width=6}
...
(Note: gene matrix is being shown on the logarithmic scale in these
plots.)

What we see here is that most of the gene expression does not change
much, except for a few genes far from the diagonal that show *large
changes* in gene expression. These are the "distinctive" or
"characteristic" genes for these factors. The point here is that
simply looking at the genes with the top expression values (the
largest elements of the ${\bf F}$ matrix) will not identify these
distinctive genes. Intuitively, this is what we are often trying to do
when performing differential expression analyses that compare
different cell types: we are most interested in identifying the genes
that show large differences in gene expression (usually large
increases), or LFCs far from zero.

TO DO: Run `de_analysis()` with `lfc.stat = "le"`, and show the
volcano plots for the three topics.

Summary: although the topic model clearly is not just doing a simple
clustering, the topics are often "cluster-like".
