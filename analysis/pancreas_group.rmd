...
A straightforward way to identify group-specific factors is to perform an ANOVA on each factor's loadings. Concretely, for the $k$th factor, we regress the loadings \(\boldsymbol{l}_k\) on the set of indicator variables \(\{\mathbb{I}(G_i = c_l)\}_{l=1}^L\):
...
For instance, \(S_k\) could be the \(R^2\) of this regression model or the \(F\)-statistic from the ANOVA test. After computing \(\{S_k\}_{k=1}^K\), we can rank the factors by their relevance to the grouping, then choose a threshold (for example, by examining an elbow plot) to automatically select the ``group-specific'' factors.
