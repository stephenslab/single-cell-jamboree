...
```{r structure-plot-topic-model-2, fig.height=1.5, fig.width=7, eval=FALSE}
...
```{r structure-plot-flashier-nmf, fig.height=1.5, fig.width=7}
rows <- order(samples$timepoint)
topic_colors <- c("darkblue","dodgerblue","darkorange","forestgreen",
                  "limegreen","tomato","darkred","olivedrab","magenta",
                  "darkmagenta","sienna","royalblue","lightskyblue",
...
p <- structure_plot(L,grouping = samples$tissue,gap = 4,
                    topics = 2:15,colors = topic_colors,
					loadings_order = rows)
print(p)
```
