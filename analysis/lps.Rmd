...
set.seed(1)
fl_nmf_cc <- flash_init(shifted_log_counts,S = s1,var_type = 2)
nn_factors <- c(1:9,12:15)
cc_factors <- 10:11
for (k in nn_factors) {
  l <- fl_nmf$L_pm[,k,drop = FALSE]
  f <- fl_nmf$F_pm[,k,drop = FALSE]
  fl_nmf_cc <- flash_factors_init(fl_nmf_cc,list(l,f),
