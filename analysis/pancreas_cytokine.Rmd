...
```{r umap-plot-1, fig.height=3, fig.width=4.75}
cluster_colors <- c("darkmagenta","royalblue","limegreen","red","darkorange")
samples$cluster                   <- "islet"      # Gcg, Sst, Ins1, Ins2, Ppy, 
                                                  # Mafa, Mafb
samples$cluster[x > 14]           <- "duct1"      # Krt19
samples$cluster[x > 12 & x < 13]  <- "duct2"      # Krt19
samples$cluster[x < -5 & y < -11] <- "macrophage" # Ccr5
samples$cluster[x > -2 & x < 5 & y < -12] <- 
...
UAMP plot for the first mouse only:

```{umap-plot-2, fig.height=3, fig.width=3.5}
...
```{r umap-plot-3, fig.height=3, fig.width=3.5}
