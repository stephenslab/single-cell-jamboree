...
U <- rsvd(shifted_log_counts[i,],k = 30)$u
```

Now run t-SNE on the 30 PCs:

```{r embeddings, results="hide", eval=FALSE}
...
```{r umap-vs-cell-type, fig.height=3.75, fig.width=5, eval=FALSE}
pdat <- cbind(samples[i,],
              data.frame(tsne1 = tsne$Y[,1],
			             tsne2 = tsne$Y[,2]))
p <- ggplot(pdat,aes(x = tsne1,y = tsne2)) +
  geom_point(size = 1.5) +
