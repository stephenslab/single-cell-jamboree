...
In this analysis, we aim to generate an improved visualization of pancreas data from the 2022 benchmarking study by [Luecken et al.][luecken-2022] using the Seurat package in R. This analysis uses the pre-processed dataset, `pancreas.RData`.
...
Set the working directory to the location of the `pancreas.RData` file, and load the dataset, which contains "counts" and "sample_info".
...
Create a Seurat object from the counts data, using `sample_info` as metadata. Normalize the data with a custom scale factor based on the mean size factor.
