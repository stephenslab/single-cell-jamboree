...
flashier_nmf <- function (X, k = 3, n.threads = 1) {
  ...
  nmf0 <- nnmf(shifted_log_counts_dense,k = 1,loss = "mse",method = "scd",
            max.iter = 10,verbose = 2,n.threads = n.threads)
  W0 <- nmf0$W
  H0 <- nmf0$H
  W0 <- cbind(W0,matrix(runif(n*(k-1)),n,k-1))
  H0 <- rbind(H0,matrix(runif(m*(k-1)),k-1,m))
  nmf <- nnmf(shifted_log_counts_dense,k,init = list(W = W0,H = H0),
              loss = "mse",method = "scd",max.iter = 10,verbose = 2,
			  n.threads = n.threads)
  x  <- rpois(1e7,1/n)
  s1 <- sd(log(x + 1))
