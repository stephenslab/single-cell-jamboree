...
Fit a topic model with $K = 13$ topics to the counts:

TO DO:

+ Switch to using K = 13 topics/factors.

+ For the Structure plots, subsample the beta cells (to make the other
  results easier to see).

+ Split the topics into two Structure plots to distinguish the
  "cell-type" topics from the other topics (since 13 topics is a lot of
  topics to distinguish in one Structure plot).
...
p <- structure_plot(tm_k12,grouping = dat$celltype,gap = 15,n = Inf)
```
...
Let's now try with $K = 12$ factors:

```{r flashier-nmf-k-12, cache=TRUE, warning=FALSE, eval=FALSE}
set.seed(1)
fl_nmf_k12 <- flash(shifted_log_counts,S = s1,ebnm_fn = ebnm_point_exponential,
                    var_type = 2,greedy_Kmax = 12,backfit = FALSE,
	   			    nullcheck = FALSE,verbose = 3)
fl_nmf_k12 <- flash_backfit(fl_nmf_k12,extrapolate = FALSE,maxiter = 40,
                            verbose = 3)
fl_nmf_k12 <- flash_backfit(fl_nmf_k12,extrapolate = TRUE,maxiter = 80,
                            verbose = 3)
```

ADD TEXT HERE.

```{r structure-plot-flashier-nmf-k-12, fig.height=2.5, fig.width=6.5, results="hide", message=FALSE, eval=FALSE}
L <- ldf(fl_nmf_k12,type = "i")$L
colnames(L) <- paste0("k",1:12)
p <- structure_plot(L[,-1],grouping = dat$celltype,gap = 15,n = Inf)
```
